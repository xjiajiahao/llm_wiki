...
This repository is a local-first LLM-maintained wiki for research knowledge across multiple domains.
...
- Default to concept-first organization. Reusable knowledge belongs in `concepts/`; instance pages should mainly capture how a concrete model, system, method, or artifact instantiates, modifies, or combines those concepts.

## Directory Conventions

- `sources/`: one page per raw source.
- `entities/`: organizations, people, products, datasets.
- `models/`: model-specific pages when a domain has named model families.
- `concepts/`: reusable technical concepts such as architectures, modules, objectives, algorithms, optimizers, geometric abstractions, and inference or search mechanisms.
...
- where it appears in specific model families, systems, or problem variants.
